...
def build_tools(config: "ToolConfig") -> list[dict]:
    ...
    tools.append(_tool(
        "control_device",
        "Control devices (lights, switches, locks, fans, blinds, covers, media_player). Use device name for fuzzy matching. Blinds/covers: open/close/raise/lower/stop/set_position/favorite. Use 'raise' or 'open' to fully open, 'lower' or 'close' to fully close. Use 'favorite' action ONLY when user literally says 'favorite position' or 'preset position'. Media: pause/resume/play/mute/unmute. ALWAYS use this for specific device commands (e.g. 'resume the TV', 'pause the TV').",
        {
            ...
            "action": {"type": "string", "enum": ["turn_on", "turn_off", "toggle", "dim", "lock", "unlock", "open", "close", "raise", "lower", "stop", "set_position", "favorite", "play", "pause", "resume", "next", "previous", "volume_up", "volume_down", "set_volume", "mute", "unmute", "set_temperature", "set_hvac_mode", "start", "dock", "locate", "return_home", "activate"], "description": "Action"},

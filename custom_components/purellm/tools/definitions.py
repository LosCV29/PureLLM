...
def build_tools(config: "ToolConfig", hass: "HomeAssistant | None" = None) -> list[dict]:
    ...
    tools.append(_tool(
        ...
        (
            ...
            + "message MUST be the user's own words. Strip the addressing part "
            "('tell Carlos') and the location part ('on the kitchen speaker'); "
            "keep EVERYTHING else exactly as said. The ONLY change allowed is "
            "flipping pronouns so it reads as spoken TO the recipient ('call "
            "him' -> 'call you'). Do not summarise, soften, censor, re-word or "
            "add commentary. NEVER put the message in brackets of any kind, "
            "never write a placeholder, and never address the recipient by "
            "name — a 'Message from <sender>' prefix is added automatically. "
            "Never read the message back. "
            "EXAMPLES (input -> arguments): "
            "'tell Carlos I'm trying to call him on the kitchen speaker' -> "
            "room='kitchen', message=\"I'm trying to call you.\"  |  "
            "'announce in the living room that dinner is ready' -> "
            "room='living room', message='Dinner is ready.'  |  "
            "'tell everyone to come downstairs' -> room='everywhere', "
            "message='Come downstairs.'  |  "
            "'say on the office speaker that I'm running late' -> "
            "room='office', message=\"I'm running late.\" "
    ...
    if config.enable_music and config.room_player_mapping:
        ...
        tools.append(_tool(
            ...
            (
                ...
                "VERBATIM TITLES: copy the title EXACTLY as the user said it. Never "
                "'correct' its spelling, grammar or word forms — song titles are full of "
                "deliberate wordplay (Wale's 'Passive Aggres-Her', 'Narciss-Her'), and "
                "'fixing' it to 'Passive Aggressive Her' searches for a song that does not "
                "exist. The search layer already handles misheard and misspelled titles. "
        ...
        tools.append(_tool(
            ...
            (
                ...
                "still resolve to the real thing — which is exactly why you must pass the title "
                "VERBATIM as the user said it. Never 'correct' its spelling, grammar or word forms: "
                "titles use deliberate wordplay (Wale's 'Passive Aggres-Her'), and rewriting it to "
                "'Passive Aggressive Her' searches for a song that does not exist. "
                "If the result says the item is not in the catalog, say so — do NOT retry with "
                "another spelling, a shorter query, or a different media_type. "
                "Returns a ranked list of candidates, each with a "
    ...
    tools.append(_tool(
        ...
        "Control lights, switches, locks, fans, covers, media_player, scripts, automations. For 'launch X' pass device=X as-is (app/script name). Only include params the user requested.",
        {
            ...
            "action": {"type": "string", "enum": ["turn_on", "turn_off", "toggle", "dim", "lock", "unlock", "open", "close", "stop", "preset", "set_position", "play", "pause", "resume", "next", "previous", "volume_up", "volume_down", "set_volume", "mute", "unmute", "set_temperature", "set_hvac_mode", "start", "dock", "locate", "return_home", "activate", "launch"], "description": "For light brightness use turn_on with brightness. set_position/position are for covers only."},
            "brightness": {"type": "integer", "description": "Light brightness percent 0-100 (NOT 0-255)."},
            "color": {"type": "string"},
